...
def build_market_prompt(data: MarketData, language: str = "zh") -> str:
    ...
    if language == "zh":
        ...
        if is_short_tf:
            ...
            lines.append("请生成一份中文字数控制在 1000 字以内（不含标点符号）的战术研报。报告必须清晰、直击要点地阐述以下 7 个核心要素：")
            lines.append("1. **市场方向**：明确给出工作周期短期趋势（看涨/看跌/震荡）以及与高周期的一致性。")
            lines.append("2. **价格是否来到关键位置**：分析价格是否测试了 Wyckoff 边界、流动性池、未触发订单块（OB）、或公允价值缺口（FVG）。")
            lines.append("3. **短期结构是否发生转变**：根据最新的 BOS/CHoCH 信号判定短期结构是延续还是发生了转变。")
            lines.append("4. **是否有 CVD 背离**：结合价格涨跌分析 CVD 是否存在明显的看涨/看跌背离及其强度。")
            lines.append("5. **是否有 OI 背离**：结合持仓量与价格走势，利用【共振黄金法则】判定是主动建仓还是被动平仓（以此识别真假突破）。")
            lines.append("6. **简洁的交易策略建议**：包含主计划和备选计划，提供具体的入场区间、止损价（SL）和止盈目标（TP），并计算盈亏比（R:R 必须合理）。")
            lines.append("7. **风险提示**：简述当前面临的最大风险（如宏观事件、费率洗盘等）。")
            lines.append("")
            lines.append("## 格式与风格约束")
            lines.append("- 第一句必须是类似：**[方向：看空 / 级别：4H] 核心战术：等待反弹，顺势做空。** 的高度概括加粗结论。")
            lines.append("- 强制采用以下 6 个 Markdown 标题进行结构化输出，直接写结论和核心数据，严禁废话：")
            lines.append("  * `### 1. 市场方向与趋势共振`")
            lines.append("  * `### 2. 关键位置与边界测试`")
            lines.append("  * `### 3. 短期结构转变 (Structure Shift)`")
            lines.append("  * `### 4. CVD 与 OI 背离研判`")
            lines.append("  * `### 5. 简洁交易策略建议`（包含主计划与备选计划的入场/止损/止盈点位）")
            lines.append("  * `### 6. 风险提示与防守红线`")
        else:
        ...
        if is_short_tf:
            ...
            lines.append("Generate a concise report up to 1000 words (excluding punctuation) directly covering the following 7 elements:")
            lines.append("1. **Market Direction**: Short-term trend of the working timeframe and its alignment with high-timeframe trend.")
            lines.append("2. **Price Location**: Whether price is testing key levels such as Wyckoff boundaries, liquidity pools, active OBs, or FVGs.")
            lines.append("3. **Structure Shift**: Determine if the short-term structure is extending or reversing via BOS/CHoCH signals.")
            lines.append("4. **CVD Divergence**: Identify bullish/bearish divergence between CVD and price.")
            lines.append("5. **OI Divergence**: Evaluate Open Interest alongside price action via the Resonance Golden Rule (accumulation vs. liquidation).")
            lines.append("6. **Concise Strategy**: Primary and alternative scenarios with Entry, Invalidation-based Stop Loss, and tiered Targets (aim for R:R >= 1:1.5).")
            lines.append("7. **Risk Warning**: Identify the most critical risk factor (e.g. macro news, funding rate contrarian washouts).")
            lines.append("")
            lines.append("## Format Constraints")
            lines.append("- Use exactly these 6 Markdown headers for structured output:")
            lines.append("  * `### 1. 市场方向与趋势共振`")
            lines.append("  * `### 2. 关键位置与边界测试`")
            lines.append("  * `### 3. 短期结构转变 (Structure Shift)`")
            lines.append("  * `### 4. CVD 与 OI 背离研判`")
            lines.append("  * `### 5. 简洁交易策略建议`")
            lines.append("  * `### 6. 风险提示与防守红线`")
            lines.append("- The very first sentence must be a bold, high-level summary. Example: **[Direction: Short / Level: 4H] Core Tactics: Sell the rally, follow the bearish momentum.**")
        else:
